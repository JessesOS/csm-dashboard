...
export async function GET(request: Request) {
  ...
  const environment = normalizeEnvironment(searchParams.get("environment") ?? defaultEnvironment);
  ...
  try {
    const tasks = await listTasks(environment, product, searchParams.get("clientId"));
...
export async function POST(request: Request) {
  try {
    ...
    const task = await createTask(payload.environment, payload.product, payload.clientId, payload);
